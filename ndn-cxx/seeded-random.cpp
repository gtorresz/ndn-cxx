...
RandomNumberEngine&
getRandomNumberEngine(unsigned int seed)
{
...
uint32_t
generateWord32(unsigned int seed)
{
  thread_local std::uniform_int_distribution<uint32_t> distribution;
  return distribution(getRandomNumberEngine(seed));
}

uint64_t
generateWord64(unsigned int seed)
{
  thread_local std::uniform_int_distribution<uint64_t> distribution;
  return distribution(getRandomNumberEngine(seed));
}

} // namespace seeded-random
} // namespace ndn
